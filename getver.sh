...
appverlist=($(python3 version-list.py "$appname"))
appver=$(dialog --backtitle "Revancify" --title $appname --no-items --ascii-lines --ok-label "Select" --menu "Select App Version" 20 40 10 "${appver[@]}" 2>&1> /dev/tty)
...
getlink=$(python3 fetch-link.py "$appname" "$appver")
clear
echo $link